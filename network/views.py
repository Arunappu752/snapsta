...
def edit_profile(request):
    if request.method == 'POST':
        ...
        if user_form.is_valid():
            new_profile_pic = request.FILES.get('new_profile_pic', None)

            if new_profile_pic:
                request.user.profile_pic = new_profile_pic

            # Check if new cover image is provided
            new_cover = request.FILES.get('new_cover', None)
            if new_cover:
                request.user.cover = new_cover
...
def followers_list(request, username):
    ...
    followers = Follower.objects.filter(user=user).values_list('followers__username', flat=True)
...
def following_list(request, username):
    user = get_object_or_404(User, username=username)
    following = Follower.objects.filter(followers=user).values_list('user__username', flat=True)
